...
public class Evaluator {
	...
	public int eval(Cons start) {
		...
		if (value.equals("(")) {
			...
			System.out.println(varlist.get(varlist.size() - 1).name + " = "
					+ varlist.get(varlist.size() - 1).value);
	...
	public int funcIf(Cons token) {
		...
		if (token.cdr.getValue().equals("(")) {
			...
			if (token.cdr.getValue().equals("(")) {
				...
				// System.out.println("IF " + nowfunc.parameter.get(0).value +
				// " = " + ans);
	...
	public int funcSetq(Cons token) {
		if (!token.cdr.getValue().equals("(")
				|| !token.cdr.getValue().equals(")")) {
			if (Character.isDigit(token.cdr.cdr.getValue().charAt(0))) {
				Variable a = new Variable(token.cdr.getValue(),
						Integer.parseInt(token.cdr.cdr.getValue()));
				varlist.add(a);
				return 1;
			}
		}
